...
/// removing elemejnt from a vector (e.g. very inefficient), this is something which will be
...
impl SharedAllocator {
    ...
    #[inline]
    pub fn shmptr_map() -> Arc<Mutex<SharedMemoryData>> {
    ...
    /// such.
    #[must_use]
    pub unsafe fn new(shmid_path: &str, size: usize) -> Self {
        ...
        // If this is first shared allocator in this process for this shared memory
            ...
            let shmid = if first {
                ...
                reset_err();
                let shmid = libc::shmget(libc::IPC_PRIVATE, full_size, libc::IPC_CREAT);
                check_err();
    ...
    /// Creates shared memory allocator when shared memory has not been previously created.
    ///
    /// # Unsafe
    ///
    /// When shared memory is for this shmid is already allocated.
    pub unsafe fn new_memory(shmid_path: &str, size: usize) -> Self {
    ...
    /// Creates shared memory allocator for a new process when a shared meory allocator for this
    /// memory has previously been created by another process.
    ///
    /// # Unsafe
    ///
    /// When used for shared memory not allocated or shared memory already attached to this process.
    pub unsafe fn new_process(shmid_path: &str) -> Self {
    ...
    fn drop(&mut self) {
        ...
        if last_process_allocator {
            ...
            if last_global_allocator {
                ...
                info!("deleting shmid file: {}", guard[index].0);
